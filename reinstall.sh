    ...
    if [[ "$REGION" == "cn" ]]; then
        ...
        case $DISTRO in
            ...
            ubuntu)
                ...
                default_mirror="archive.ubuntu.com"
                ;;
            centos)
                overseas_mirrors=(
                ...
                default_mirror="mirror.stream.centos.org"
                ;;
            almalinux)
                overseas_mirrors=(
                    "repo.almalinux.org"
                ...
                default_mirror="repo.almalinux.org"
                ;;
            rockylinux)
                overseas_mirrors=(
                ...
                default_mirror="download.rockylinux.org"
                ;;
            fedora)
                overseas_mirrors=(
                ...
                default_mirror="download.fedoraproject.org"
                ;;
            alpine)
                overseas_mirrors=(
                ...
                default_mirror="dl-cdn.alpinelinux.org"
                ;;
            *)
...
EOF
    ...
    if command -v grub2-mkconfig &>/dev/null; then
        ...
        # 设置下次启动进入 DD Reinstall
        if command -v grub2-reboot &>/dev/null; then
            grub2-reboot "DD Reinstall" 2>/dev/null || true
        fi
        if command -v grub2-set-default &>/dev/null; then
            grub2-set-default "DD Reinstall" 2>/dev/null || true
        fi
    elif command -v grub-mkconfig &>/dev/null; then
        grub-mkconfig -o /boot/grub/grub.cfg 2>/dev/null
        # 设置下次启动进入 DD Reinstall
        if command -v grub-reboot &>/dev/null; then
            grub-reboot "DD Reinstall" 2>/dev/null || true
        fi
        if command -v grub-set-default &>/dev/null; then
            grub-set-default "DD Reinstall" 2>/dev/null || true
        fi
    fi
    ...
    if [[ -f /etc/default/grub ]]; then
        ...
        # 设置默认启动项为 DD Reinstall
        sed -i 's/^GRUB_DEFAULT=.*/GRUB_DEFAULT="DD Reinstall"/' /etc/default/grub
        # 禁用超时隐藏
