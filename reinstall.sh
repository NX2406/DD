...
select_mirror() {
    if [[ "$REGION" == "cn" ]]; then
        ...
        case $DISTRO in
            debian) SELECTED_MIRROR=$MIRROR_DEBIAN ;;
            ubuntu) SELECTED_MIRROR=$MIRROR_UBUNTU ;;
...
input_user_config() {
    ...
    # DNS 设置
    echo -e "${CYAN}设置 DNS (默认: 8.8.8.8,8.8.4.4):${NC}"
    read -r -p ">>> " input_dns
    if [[ -n "$input_dns" ]]; then
        IFS=',' read -r DNS1 DNS2 <<< "$input_dns"
    fi
    echo ""
...
install_debian_ubuntu() {
    ...
    if [[ "$DISTRO" == "debian" ]]; then
        ...
        if [[ "$REGION" == "cn" ]]; then
            base_url="https://${SELECTED_MIRROR}/debian/dists/${codename}/main/installer-${ARCH}/current/images/netboot/debian-installer/${ARCH}"
        else
            base_url="https://deb.debian.org/debian/dists/${codename}/main/installer-${ARCH}/current/images/netboot/debian-installer/${ARCH}"
        fi
    else
        if [[ "$REGION" == "cn" ]]; then
            base_url="https://${SELECTED_MIRROR}/ubuntu/dists/${VERSION_ID}/main/installer-${ARCH}/current/legacy-images/netboot/ubuntu-installer/${ARCH}"
        else
            base_url="https://archive.ubuntu.com/ubuntu/dists/${VERSION_ID}/main/installer-${ARCH}/current/legacy-images/netboot/ubuntu-installer/${ARCH}"
        fi
    fi
